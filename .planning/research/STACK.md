...
- ~~WebSearch — Gemini 2.5 Flash-Lite free tier: ~30 RPM / 1,500 RPD figures cited by secondary sources. **LOW-MEDIUM confidence**~~ → **CORREGIDO Y MEDIDO EN VIVO (2026-07-10): el free tier permite 15 RPM, no 30.** Confirmado contra la API real durante `scripts/verify-bot-conversation-live.ts`: HTTP 429, `"status": "RESOURCE_EXHAUSTED"`, `quotaId: "GenerateRequestsPerMinutePerProjectPerModel-FreeTier"`, `quotaValue: "15"`, `model: "gemini-3.1-flash-lite"`. **HIGH confidence** — es la respuesta de la propia API, no una fuente secundaria.
  - Implicancia de capacidad: el tool-loop del bot consume **1-3 requests por mensaje del cliente** (buscarHorarios + consultarNegocio + la narración, más el reintento del guard de empty-text). 15 RPM se agota con ~5-8 mensajes por minuto **sumando todos los tenants**. Presupuestar el tier pago antes del primer cliente real con volumen.
  - Nota: el código usa `gemini-3.1-flash-lite`, no `gemini-2.5-flash-lite`. Esta tabla y `CLAUDE.md` todavía dicen 2.5.

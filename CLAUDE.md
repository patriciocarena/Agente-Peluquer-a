# ⚠️ AISLAMIENTO DE PROYECTO — REGLA DURA

Este proyecto (TurnosBot / carpeta `Agente Peluqueria`) **NO tiene ninguna relación** con el proyecto del restaurante. Trabajando en esta carpeta:

- **NUNCA mezclar con el proyecto del restaurante** (Supabase `hzgunbftloevclkohcdf`: tablas `restaurants`, `menu_items`, `orders`, `call_logs`, etc.). Ni leer, ni escribir, ni copiar patrones, ni aplicar migraciones ahí.
- La **única** base de datos de este proyecto es Supabase **`bdgufnitakelyialjoqg`** (`https://bdgufnitakelyialjoqg.supabase.co`). Credenciales en `.env` (gitignored).
- **MCP de Supabase (actualizado 2026-07-04):** el MCP global (`~/.claude.json` → `mcpServers.supabase`, prefix `mcp__supabase__*`) fue **repuntado a `bdgufnitakelyialjoqg`** (antes apuntaba al restaurante `hzgunbftloevclkohcdf`). Efectivo tras reiniciar Claude Code — en una sesión ya abierta antes del cambio, ese MCP sigue conectado al restaurante hasta el restart, así que verificá el ref antes de usarlo. Alternativas siempre válidas y aisladas: `@supabase/supabase-js` con las keys del `.env`, o la Management API con el PAT del `.env` contra `bdgufnitakelyialjoqg` (`POST https://api.supabase.com/v1/projects/bdgufnitakelyialjoqg/database/query`, con `curl` — el urllib de Python lo bloquea Cloudflare). **Nunca** apuntar ninguna herramienta al ref del restaurante.
- **TODO SE CONSTRUYE DE CERO.** No reutilizar nada de otros proyectos: ni schema, ni código, ni componentes, ni patrones, ni migraciones. Greenfield puro. El único material de referencia válido es el de este repo (`.planning/`, el schema de referencia de 16 tablas descrito en la investigación del propio proyecto).
...
**Core Value:** Un cliente puede agendar un turno real, en un horario realmente disponible, conversando por WhatsApp en lenguaje natural — sin intervención humana de la peluquería. Si eso funciona de forma confiable, el producto tiene valor; todo lo demás es secundario.
